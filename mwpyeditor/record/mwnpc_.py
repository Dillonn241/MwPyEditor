...
class MwNPC_(MwRecord):
    def __init__(self):
        ...
        self.respawn = False
        self.autocalc = False
        self.auto_attributes = None
        self.auto_skills = None
        self.auto_health = None
        self.auto_magicka = None
        self.auto_fatigue = None
        self.auto_reputation = None
    ...
    def autocalc_stats(self):
        if not self.autocalc:
            self.auto_attributes = self.attributes
            self.auto_skills = self.skills
            self.auto_health = self.health
            self.auto_magicka = self.magicka
            self.auto_fatigue = self.fatigue
            self.auto_reputation = self.reputation
            ...
            for mw_skill in mwglobals.records['SKIL']:
                if mw_skill.governing_attribute_id == attribute_id:
                    if mw_skill.id_ in mw_class.major_skill_ids:
                        ...
                        k += 0.2
            self.auto_attributes.append(round(base + k * (self.level - 1)))
        ...
        if 5 in mw_class.primary_attribute_ids:  # Endurance
            health_mult += 1
        str_end_average = 0.5 * (self.auto_attributes[0] + self.auto_attributes[5])  # Strength + Endurance
        self.auto_health = math.floor(str_end_average + health_mult * (self.level - 1))

        self.auto_magicka = math.floor(2 * self.auto_attributes[1])  # fNPCbaseMagickaMult * Intelligence
        # Strength + Willpower + Agility + Endurance
        self.auto_fatigue = (self.auto_attributes[0] + self.auto_attributes[2] + self.auto_attributes[3] +
                             self.auto_attributes[5])
        ...
        for mw_skill in mwglobals.records['SKIL']:
            ...
            self.auto_skills.append(round(base + k * (self.level - 1)))

        if self.faction:
            self.auto_reputation = 2 * (self.faction_rank + 1)  # 2 = iAutoRepFacMod
            # formula technically adds
            # iAutoRepLevMod [0] * (level - 1)
            # which is always 0
        else:
            self.auto_reputation = 0
    ...
    def get_attribute(self, attribute_name):
        if attribute_name in mwglobals.ATTRIBUTES:
            ...
            return self.auto_attributes[attribute_id]
    
    def get_attributes_dict(self):
        return {mwglobals.ATTRIBUTES[attribute_id]: self.auto_attributes[attribute_id]
                for attribute_id in range(8)}

    def get_skill(self, skill_name):
        if skill_name in mwglobals.SKILLS:
            skill_id = mwglobals.SKILLS.index(skill_name)
            return self.auto_skills[skill_id]
    
    def get_skills_dict(self):
        return {mwglobals.SKILLS[skill_id]: self.auto_skills[skill_id]
