...
class MwREGN(MwRecord):
    ...
    def load(self):
        ...
        for i in range(self.num_subrecords("WEAT")):
            self.weather_chances[mwglobals.WEATHERS[i]] = self.get_subrecord_uint("WEAT", index=i, start=i, length=1)
        self.sleep_creature = self.get_subrecord_string("BNAM")
        self.map_red = self.get_subrecord_uint("CNAM", start=0, length=1)
        self.map_blue = self.get_subrecord_uint("CNAM", start=1, length=1)
        self.map_green = self.get_subrecord_uint("CNAM", start=2, length=1)
        self.sounds = {}
        for i in range(self.num_subrecords("SNAM")):
            sound_id = self.get_subrecord_string("SNAM", index=i, start=0, length=16)
            sound_chance = self.get_subrecord_uint("SNAM", index=i, start=32, length=1)
    ...
    def diff(self, other):
        MwRecord.diff(self, other, ["name", "weather_chances", "sleep_creature", "map_red", "map_blue", "map_green",
                                    "sounds"])
